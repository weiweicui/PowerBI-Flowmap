...
export class Banner<T> {
    private _root = null as ISelex;
    private _origin = null as Func<any, IPoint>;
    private _key = null as Func<any, string>;
    private _content = null as Func<any, HTMLElement>;
    private _data = {} as StringMap<{ tip: ISelex, value: any, rect: IRect, pos: string }>;
    constructor(root: ISelex, color = [242, 200, 17]) {
        this._root = root;
        this._rgb = color.join(', ');
    }

    public _background = null as Func<T, number[]>;
    public background(conv: Func<T, number[]>) {
        this._background = conv;
    }
    ...
    public add(v: T, pos = 'top' as 'top' | 'bottom' | 'bot' | 'left' | 'right'): void {
        if (pos === 'bot') {
            pos = 'bottom';
        }
        if (!v) {
    ...
    private _arrange(tip: ISelex, v: any, pos: string, rect?: IRect): IRect {
    ...
    private _shift(tip: ISelex, min: number, max: number, v: number, size: number, tag: string) {
        if (v - size / 2 < min) {
            if (min < v - 7) {
                var delta = v - 7 - min;
    ...
    private _color(v: T): string {
        let color = this._background ? this._background(v).join(',') : this._rgb;
        return `rgba(${color}, ${this._opacity})`;
    }

    private _newTip(v: T, pos: string): ISelex {
        var tip = selex(document.createElement('div')).classed('tip ' + pos, true);
        var color = this._color(v);
    ...
    public opacity(v: number): this {
        ...
        this._opacity = v;
        this._root.selectAll<T>('.content-wrap').style('background-color', v => this._color(v));
        for (var loc of ['top', 'left', 'right', 'bottom']) {
            this._root.selectAll<T>('.arrow.' + loc).style('border-' + loc + '-color', v => this._color(v));
        }
        return this;
    }
}