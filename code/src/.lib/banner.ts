...
export class Banner<T> {
    ...
    private _data = {} as StringMap<{ tip: Any, value: any, rect: IRect, pos: string }>;
    ...
    public add(v: T, pos = 'top' as 'top' | 'bottom' | 'bot' | 'left' | 'right') {
        if (pos === 'bot') {
            pos = 'bottom';
        }
        var key = this._key(v);
    ...
    private _newTip(v: T, pos: string): d3.Any {
        var tip = select(document.createElement('div')).classed('tip ' + pos, true);
    ...
    public opacity(v: number): this {
        ...
        this._opacity = v;
        this._root.selectAll<T>('.content-wrap').style('background-color', v => {
            return this._color(v);
        });
        for (var loc of ['top', 'left', 'right', 'bottom']) {
            this._root.selectAll<T>('.arrow.' + loc)
                .style('border-' + loc + '-color', v => this._color(v));
        }
        return this;
    }
}